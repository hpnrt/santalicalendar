...
class Calendar {
    ...
    public function __construct(){
        ...
        $this->santaliDate = new santaliDate();
    }

    /********************* PROPERTY ********************/
    //private $dayLabels_english = array("Sun","Mon","Tue","Wed","Thu","Fri","Sat");
    private $dayLabels = array( "&#7269;&#7268;&#7288;&#7260;&#7278;" // Sunday
                                "&#7258;&#7259;&#7278;", //Monday
                                "&#7285;&#7263;&#7262;&#7278;", // Tuesday
                                "&#7269;&#7263;&#7289;&#7260;&#7273;&#7281;", // Wednesday
                                "&#7269;&#7263;&#7289;&#7272;&#7275;&#7268;", // Thurseday
    ...
    private $EAPInfo = array(); // Muluh, Kunami, Uwans

    private $santaliDate=null;
    ...
    */
    public function show() {
        ...
        $content.='</div>';
        $content.='<table id="calenderdates" class="table table-bordered santaliTable" style="height:70%;"><thead><tr>'.$this->_createLabels().'</tr></thead>';
    ...
    **/
    private function _populateHoliday($month, $year){
        ...
        $EAPs = $db->fetchMuluhKunamiUwans ($month, $year);
        $EAP = explode("|", $EAPs);
    ...
    */
    private function _createNavi(){
        ...
        $preMonthsantali = $this->santaliDate->getMonth(date('n',strtotime($preYear.'-'.$preMonth.'-1')));

        // This month and year
        $thisYearsantali = $this->santaliDate->getMahit(date('Y',strtotime($this->currentYear.'-'.$this->currentMonth.'-1')));
        $thisMonthsantali = $this->santaliDate->getMonth(date('n',strtotime($this->currentYear.'-'.$this->currentMonth.'-1')));

        // Next month and year
        $nextMonthsantali = $this->santaliDate->getMonth(date('n',strtotime($nextYear.'-'.$nextMonth.'-1')));

        // Previous month button
        $retVal ='<div class="col-sm-2" style="display:block;cursor:pointer;text-decoration:none;color:#FFF;">'.
                //'<a class="btn btn-info input-block-level pull-left" href="'.$this->naviHref.'?month='.sprintf('%02d',$preMonth).'&year='.$preYear.'"><span class="glyphicon glyphicon-chevron-left"></span>  &#7266;&#7263;&#7276;&#7263;&#7261;  </a>'.
                    '<a class="btn btn-info btn-block" href="'.$this->naviHref.'?month='.sprintf('%02d',$preMonth).'&year='.$preYear.'" style="font-weight: 600"><span class="glyphicon glyphicon-chevron-left"></span>  '.$preMonthsantali.'  </a>'.
                '</div>';

        // Middle label
        $retVal .='<div class="col-sm-8 text-center">'.
                      '<a href="#" class="btn btn-success btn-block active" background-color="#cccccc">'.$thisMonthsantali.' '.$thisYearsantali.'</a>'.
                  '</div>';

        // Next Month button
        $retVal .='<div class="col-sm-2" style="display:block;cursor:pointer;text-decoration:none;color:#FFF;">'.
                //'<a class="btn btn-info input-block-level pull-right" href="'.$this->naviHref.'?month='.sprintf("%02d", $nextMonth).'&year='.$nextYear.'">  &#7259;&#7263;&#7277;&#7258;&#7266;  <span class="glyphicon glyphicon-chevron-right"></span></a>'.
                      '<a class="btn btn-info btn-block pull-right" href="'.$this->naviHref.'?month='.sprintf("%02d", $nextMonth).'&year='.$nextYear.'">  '.$nextMonthsantali.'  <span class="glyphicon glyphicon-chevron-right"></span></a>'.
                  '</div>';
    ...
    **/
    private function _showHolidays () {
        ...
        $retVal.='             <h3 id="holidaylist" class="panel-title">&#7279;&#7263;&#7272;&#7263;&#7285;&#7293;&#32;&#7279;&#7273;&#7281;&#7263;&#7289;&#7277;</h3>';
        $retVal.='         </div>';
        ...
        foreach( $this->holidayInfo as $key => $val ) {
            if (!empty ($val)) {
                ...
                $retVal.='  <div class="col-md-3">'.$this->santaliDate->getMahit($key).'</div>';
    ...
    private function _showDay($cellNumber){
        ...
        */
        if ($isHoliday) {
            ...
            $return.= $this->santaliDate->getMahit ($cellContent);
